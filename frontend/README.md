...
节点标签会根据左右可用空间自动选择方向，并按空间截短；悬停卡片会根据节点位置自动避让，不会越过地图边界。算法参数继续由 `garden.json` 暴露，供外部页面或 API 使用。
...
- 悬停/聚焦节点：查看日期、摘要、标签和封面预览；
- 点击节点：显示标题、摘要、标签和 4 篇附近文章，同时记录本次探索路径；从详情卡片打开原文。
...
| `theme` | `?theme=obsidian` | 使用 Obsidian 风格深色工作区 |
